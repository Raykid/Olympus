 ...
 */
export function compile(comp, currentTarget, envModels) {
    // 取到编译参数列表
    var bindParams = currentTarget.__bind_commands__;
    // 编译currentTarget自身
    if (bindParams) {
        ...
        for (var i = 0; i < bindParams.length;) {
            ...
            // 调用编译命令，并且更新中止状态
            params.cmd.apply(params, [comp, currentTarget, params.target, envModels || []].concat(params.args));
        }
    }
}
/**
 ...
 */
export function compileValue(comp, currentTarget, target, envModels, name, exp) {
    bindValue(comp, currentTarget, target, envModels, name, exp);
}
/**
 * 编译bindExp命令，不会中止编译
 */
export function compileExp(comp, currentTarget, target, envModels, exp) {
    bindExp(comp, currentTarget, target, envModels, exp);
}
/**
 * 编译bindFunc命令，不会中止编译
 */
export function compileFunc(comp, currentTarget, target, envModels, name) {
    var argExps = [];
    for (var _i = 5; _i < arguments.length; _i++) {
        argExps[_i - 5] = arguments[_i];
    }
    bindFunc.apply(void 0, [comp, currentTarget, target, envModels, name].concat(argExps));
}
/**
 * 编译bindOn命令，不会中止编译
 */
export function compileOn(comp, currentTarget, target, envModels, type, exp) {
    bindOn(comp, currentTarget, target, envModels, type, exp);
}
function isPosterity(comp, target, parent) {
    var tempParent = comp.bridge.getParent(target);
    if (!tempParent)
        return false;
    else if (tempParent === parent)
        return true;
    else
        return isPosterity(comp, tempParent, parent);
}
function getAllSubTargets(comp, target) {
 ...
 */
export function compileIf(comp, currentTarget, target, envModels, exp) {
    // 将后面的编译命令缓存起来
    var bindParams = currentTarget.__bind_commands__;
    var caches = [{ target: currentTarget, params: bindParams.splice(0, bindParams.length) }];
    // 后代节点的也要缓存住
    var subTargets = getAllSubTargets(comp, currentTarget);
    ...
    bindIf(comp, currentTarget, target, envModels, exp, function (value) {
 ...
 */
export function compileFor(comp, currentTarget, target, envModels, name, exp, compCls, declaredComponentCls, dataExp) {
    // 将后面的编译命令缓存起来
    var leftHandlers = target.__stop_left_handlers__;
    // 绑定for命令
    bindFor(comp, currentTarget, target, envModels, name, exp, compCls, declaredComponentCls, dataExp, function (data, renderer, subEnvModels) {
        ...
        for (var depth in bindTargets) {
            ...
            dict.forEach(function (currentTarget) { return compile(comp, currentTarget, subEnvModels); });
        }
    });
}
/**
