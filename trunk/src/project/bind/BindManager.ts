...
import { createRunFunc, EvalExp, evalExp } from "../../core/bind/Utils";
import { IWatcher } from "../../core/bind/Watcher";
...
export default class BindManager
{
     ...
     * @param {IComponentConstructor} [declaredMediatorCls] 声明的Mediator类型
     * @param {string} [dataExp] 提供给中介者包装器的数据表达式
     * @param {(data:any, renderer:any, envModels:any[])=>void} [callback] 每次生成新的renderer实例时调用这个回调
     * @memberof BindManager
     */
    public bindFor(mediator:IMediator, currentTarget:any, target:any, envModels:any[], name:string, exp:string, mediatorCls?:IComponentConstructor, declaredMediatorCls?:IComponentConstructor, dataExp?:string, callback?:(data:any, renderer:any, envModels:any[])=>void):void
    {
        var watcher:IWatcher;
        var bindData:BindData = this._bindDict.get(mediator);
        var replacer:any = mediator.bridge.createEmptyDisplay();
