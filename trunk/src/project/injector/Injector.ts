...
export function SubMediator(mediatorCls:IComponentConstructor, dataExp?:string):PropertyDecorator;
export function SubMediator(skin:any, mediatorCls?:IComponentConstructor, dataExp?:string):PropertyDecorator;
...
{
    var oriSkin:any;
    var mediatorCls:IComponentConstructor;
    var dataExp:string;
    // 判断是否是参数化装饰
    if(this === decorateThis)
    {
 ...
 * @param {IComponentConstructor} [mediatorCls] 提供该参数将使用提供的中介者包装每一个渲染器
 * @param {string} [dataExp] 传递给中介者的数据表达式
 * @returns {PropertyDecorator} 
 */
export function BindFor(exp:string, mediatorCls?:IComponentConstructor, dataExp?:string):PropertyDecorator;
/**
 * 绑定数据集合到指定对象
 * 
 * @export
 ...
 * @returns {PropertyDecorator} 
 */
export function BindFor(name:string, exp:string, mediatorCls?:IComponentConstructor, dataExp?:string):PropertyDecorator;
/**
 * @private
 */
