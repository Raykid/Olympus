...
namespace olympus
{
     ...
     * @param {string} [version] 加载version.cfg文件的版本号，不传则使用随机时间戳作为版本号
     */
    export function preload(jsFiles:JSFile[], host?:string, callback?:(err?:Error)=>void, ordered:boolean=true, version?:string):void
    {
        // 首先初始化version
        _version.initialize(()=>{
    ...
    function loadJsFiles(jsFiles:JSFile[], host:string, callback:(err?:Error)=>void, ordered:boolean):void
    {
        ...
        function onJSONPLoadOne(evt:Event):void
        {
            ...
            var success:(evt:Event)=>void = function(evt:Event):void
            {
                // 取到索引
        ...
        function onJSONPLoadError():void
        {
            // 调用失败
            onErrorOne();
        }
