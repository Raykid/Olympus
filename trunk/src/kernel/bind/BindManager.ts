...
{
    private _bindDict:Dictionary<IComponent, BindData> = new Dictionary();
     ...
     */
    public bind(comp:IComponent):Bind
    {
        var bindData:BindData = this._bindDict.get(comp);
        if(!bindData)
        {
            this._bindDict.set(comp, bindData = {
                bind: new Bind(comp),
                callbacks: []
            });
        }
     ...
     * @memberof BindManager
     */
    public unbind(comp:IComponent):Bind
    {
        var bindData:BindData = this._bindDict.get(comp);
        if(bindData)
        {
            bindData.bind.dispose();
            this._bindDict.delete(comp);
        }
        return bindData && bindData.bind;
    }

    private addBindHandler(comp:IComponent, callback:()=>void):void
    {
        var handler:()=>void = ()=>{
            // 判断数据是否合法
     ...
     * @memberof BindManager
     */
    public bindValue(comp:IComponent, currentTarget:any, target:any, envModels:any[], name:string, exp:EvalExp):void
    {
        var watcher:IWatcher;
        var bindData:BindData = this._bindDict.get(comp);
        this.addBindHandler(comp, ()=>{
            // 如果之前绑定过，则要先销毁之
     ...
     * @memberof BindManager
     */
    public bindExp(comp:IComponent, currentTarget:any, target:any, envModels:any[], exp:EvalExp):void
    {
        var watcher:IWatcher;
        var bindData:BindData = this._bindDict.get(comp);
        this.addBindHandler(comp, ()=>{
     ...
     * @memberof BindManager
     */
    public bindFunc(comp:IComponent, currentTarget:any, target:any, envModels:any[], name:string, ...argExps:(EvalExp)[]):void
    {
     ...
     * @memberof BindManager
     */
    public bindOn(comp:IComponent, currentTarget:any, target:any, envModels:any[], type:string, exp:EvalExp):void
    {
        this.addBindHandler(comp, ()=>{
            var commonScope:any = {
                $this: comp,
            ...
            var nearestAncestor:any = this.getNearestAncestor(comp.bridge, target, "__bind_sub_events__");
     ...
     * @memberof BindManager
     */
    public bindIf(comp:IComponent, currentTarget:any, target:any, envModels:any[], exp:EvalExp, callback?:(value:boolean)=>void):void
    {
        var watcher:IWatcher;
        var bindData:BindData = this._bindDict.get(comp);
     ...
     * @memberof BindManager
     */
    public bindFor(comp:IComponent, currentTarget:any, target:any, envModels:any[], name:string, exp:string, compCls?:IComponentConstructor, declaredComponentCls?:IComponentConstructor, dataExp?:string, callback?:(data:any, renderer:any, envModels:any[])=>void):void
    {
        var watcher:IWatcher;
        var bindData:BindData = this._bindDict.get(comp);
...
/** 再额外导出一个单例 */
export const bindManager:BindManager = new BindManager();