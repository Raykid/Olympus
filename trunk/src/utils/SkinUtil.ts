 ...
 */
export function replaceSkin(bridge:IBridge, newSkin:any, oldSkin:any):void
{
    // 两个皮肤必须都是隶属桥的皮肤
    if(bridge.isMySkin(newSkin) && bridge.isMySkin(oldSkin))
    {
        var parent:any = bridge.getParent(oldSkin);
        if(parent)
        {
            var index:number = bridge.getChildIndex(parent, oldSkin);
