    ...
    public bindFor(mediator:IMediator, currentTarget:any, target:any, envModels:any[], name:string, exp:BindForExpType, mediatorCls?:IMediatorConstructor, declaredCls?:IConstructor, dataExp?:EvalExp, callback?:(data:any, renderer:any, envModels:any[])=>void):void
    {
        ...
        this.addBindHandler(mediator, ()=>{
            ...
            if(typeof exp === "string")
            {
                ...
                if(!res) return;
                expTuple = [res[1], <"in"|"of">res[2], res[5]];
            }
            else
            {
                // 元组形式，直接赋值
            ...
            if(watcher) watcher.dispose();
            // 获得要遍历的数据集合
            watcher = bindData.bind.createWatcher(currentTarget, target, expTuple[2], (datas:any)=>{
...
/** BindFor三元组形式 */
export type BindForExpTuple = [string, "in"|"of", EvalExp];

/** BindFor表达式可接受的类型 */
export type BindForExpType = string | BindForExpTuple;
