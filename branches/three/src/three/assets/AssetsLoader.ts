...
import { Camera, FileLoader, ObjectLoader, Scene } from "three";
 ...
 * @returns {IResource} 
 */
export function getCache(url:string):IResource
{
    return cache[url];
}
                    ...
                    try
                    {
                        ...
                        switch(json.metadata.type)
                        {
                            case "Object":
                                ...
                                new ObjectLoader().parse(json, (scene:Scene)=>{
                                    // 填充缓存&配置
                                    cache[url] = dict[url] = {
                                        scene: scene,
                                        configText: text
                            ...
                            case "App":
                                ...
                                new ObjectLoader().parse(json.scene, (scene:Scene)=>{
                                    // 再用ObjectLoader解析摄像机
